...
app.post('/api3/lideres_incidencias', async (req, res) => {
  try {
    let { nombre, area, activo, correo, jerarquia } = req.body;
    ...
    const result = await pool.query(
      `INSERT INTO public.lideres_incidencias (nombre, area, activo, correo, jerarquia)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [String(nombre).trim(), String(area).trim(), activo, correo, jerarquia]
    );
...
app.put('/api3/lideres_incidencias/:id', async (req, res) => {
  ...
  try {
    ...
    let { nombre, area, activo } = req.body;
    ...
    const upd = await pool.query(
      `UPDATE public.lideres_incidencias
         SET nombre = $1, area = $2, activo = $3, updated_at = NOW()
       WHERE id = $4
       RETURNING *;`,
      [nuevoNombre, nuevaArea, activo, id]
    );
